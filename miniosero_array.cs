...
namespace miniosero_array
{
    public partial class Form1 : Form
    {
        ...
        int i;
        int turn;
        int[] button_color_index = { 0, 0, 0, 0, 0, 0 };
        ...
        public void Button_click(object sender, EventArgs e)
        {
            ...
            switch(turn)
            {
                case 0:   //white turn
                    ...
                    {
                        if ((button_color_index[g] == 1) || (button_color_index[g] == 0)||(g == buttons.Count -1))  //両端が同じ色、または押されてないならbreak
                        {
                            break;
                        }
                        if (button_color_index[g] == 2)　　　//右側が違う色ならその先に白があったらひっくり返る
                        {
                            ...
                            {
                                if (button_color_index[gg] == 1)
                                {
                                    for (int s = g; s <= gg - 1; s++)
                                    {
                                        button_color_index[s] = 1;
                    ...
                    for (int h = btn.TabIndex -1; h >= 0; h--)  //左に色をチェック
                    {
            ...
            label5.Text = Convert.ToString(blackpoint.Sum());
        ...
        {
            if(button_sum.Sum()==buttons.Count)
            {
        ...
        {
            if (blackpoint.Sum() > whitepoint.Sum())
            {
                textBox1.Text = "黒の勝ち";
            }
